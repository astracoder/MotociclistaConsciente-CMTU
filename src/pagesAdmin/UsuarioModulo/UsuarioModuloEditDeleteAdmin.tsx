...
export const UsuarioModuloEditDeleteAdmin = () => {
  ...
  const handleSalvar = async () => {
    ...
    try {
      const response = await axios.put(`http://${ipconfig}:3000/usuarioModulo/editarUsuarioModulo`, {
  ...
  const handleAtivar = async () => {
    try {
      const response = await axios.put(`http://${ipconfig}:3000/usuarioModulo/ativarUsuarioModulo`, {id});

      if (response.status === 200) {
        navigation.navigate('UsuarioModuloAdmin');
        alert('Usuário módulo ativado com sucesso!');
      }
    } catch(err) {
  ...
  const handleDesativar = async () => {
    try {
      const response = await axios.put(`http://${ipconfig}:3000/usuarioModulo/desativarUsuarioModulo`, {id});

      if (response.status === 200) {
        navigation.navigate('UsuarioModuloAdmin');
        alert('Usuário módulo desativado com sucesso!');
      }
    } catch(err) {
